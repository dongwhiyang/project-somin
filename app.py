...
if st.session_state.phase == 0:
    ...
    if st.session_state.topics_data is None:
        ...
        with col_c1:
            if st.button("🧠 AI 주제 분석 시작", use_container_width=True):
        ...
        with col_c2:
            if st.button("🚀 자동 주제 선정 및 즉시 발행", use_container_width=True):
        ...
        with col_ca:
            if st.button("🚀 커스텀 주제로 즉시 발행", use_container_width=True):
    ...
    with col2:
        if st.button("🚀 원스톱 블로그 자동 생성", use_container_width=True):
